...
constexpr long long nmaxevents = test?5000:50000; // to avoid large-size figures for 2D plots and long running time

//
std::string sel_ = "(1)";
// std::string sel_ = "(fabs(vz[1]) < 15 && cluscomp && nhfp > 1 && nhfn > 1)";
constexpr bool labelid = false;
constexpr bool recreate = true;
...
static const std::vector<varinfo_t> options_pixel_2d = {
  ...
  //    {"eta@", "cs@"},
  //    {{200, -4, 4}, {40, 0, 40}},
  //    {600, 600}, 0x03, sel_, "colz", nmaxevents
  // },
  {
    "z-phi", {"z", "#phi"},
    {"r@/tan(2*atan(exp(-eta@)))", "phi@"},
    {{1000, -30, 30}, {1000, -4, 4}},
    {600, 2400}, 0x01, sel_, "colz", nmaxevents
  },
  {
    "fpix-x-y-plus", {"x", "y"},
    {"r@*cos(phi@)", "r@*sin(phi@)"},
    {{1000, -20, 20}, {1000, -20, 20}},
    {600, 600}, 0x02, "(eta@>0)", "colz", nmaxevents
  }, {
    "fpix-x-y-minus", {"x", "y"},
    {"r@*cos(phi@)", "r@*sin(phi@)"},
    {{1000, -20, 20}, {1000, -20, 20}},
  ...
  //    {"eta@", "r@"},
  //    {{1000, -4, 4}, {1000, 0, 20}},
  //    {600, 600}, 0x33, sel_, "colz", nmaxevents
  // }, {
  //    "x-y", {"x", "y"},
  //    {"r@*cos(phi@)", "r@*sin(phi@)"},
  //    {{1000, -20, 20}, {1000, -20, 20}},
  //    {600, 600}, 0x11, sel_, "colz", nmaxevents
  // }, {
  //    "z-r", {"z", "r"},
  //    {"r@/tan(2*atan(exp(-eta@)))", "r@"},
  //    {{1000, -60, 60}, {1000, 0, 20}},
  //    {1200, 600}, 0x33, sel_, "colz", nmaxevents
  // }, {
  //    "fpix-z-phi", {"z", "#phi"},
  //    {"r@/tan(2*atan(exp(-eta@)))", "phi@"},
  //    {{1000, -60, 60}, {1000, -4, 4}},
  //    {1200, 600}, 0x22, sel_, "colz", nmaxevents
  // }
};
...
static const std::vector<varinfo_t> options_compare_pixel_2d = {
  {
    ...
    {"r@*cos(phi@)", "r@*sin(phi@)"},
    {{1000, -20, 20}, {1000, -20, 20}},
    {600, 600}, 0x02, Form("(%s && eta@>0)", sel_.c_str()), "colz", nmaxevents
  }, {
    "fpix-x-y-minus", {"x", "y"},
    {"r@*cos(phi@)", "r@*sin(phi@)"},
    {{1000, -20, 20}, {1000, -20, 20}},
...
static const std::vector<varinfo_t> options_tracklet_2d = {
  {
    ...
    {"eta1", "phi1"},
    {{1000, -4, 4}, {1000, -4, 4}},
    {600, 600}, 0, sel_, "colz", nmaxevents
  }, {
    "eta-vz", {"#eta", "v_{z}"},
    {"eta1", "vz[1]"},
    {{200, -4, 4}, {200, -20, 20}},
